...
public class LazySitioDataModel extends LazyDataModel<Sitio> implements Serializable {

    @Inject
    private Logger log;
    // Data Source for binding data to the DataTable
    private List<Sitio> datasource;
     ...
     * @param crudService
     */
    public LazySitioDataModel(DataAccessService crudService) {
     ...
     */
    @Override
    public List<Sitio> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
        CriteriaBuilder criteriaBuilder = crudService.getCriteriaBuilder();
        CriteriaQuery<Sitio> criteriaQuery = criteriaBuilder.createQuery(Sitio.class);
        Metamodel metamodel = crudService.getMetamodel();
        EntityType<Sitio> entityType = metamodel.entity(Sitio.class);
        Root<Sitio> from = criteriaQuery.from(entityType);
        List<Order> orders = new ArrayList<Order>();

        // count query
        CriteriaQuery<Long> cqCount = criteriaBuilder.createQuery(Long.class);
        Root<Sitio> fromCount = cqCount.from(entityType);
...
/*
    ...
    public Sitio getRowData(String rowKey) {
        if (datasource == null)
            return null;
        for (Sitio sitio : datasource) {
            if (sitio.getId().toString().equals(rowKey))
                return sitio;
        }
        return null;
    }
    ...
    public void setWrappedData(Object list) {
        this.datasource = (List<Sitio>) list;
    }
}
