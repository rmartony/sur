...
public class LazyTipoAntenaDataModel extends LazyDataModel<TipoAntena> implements Serializable {

    // Data Source for binding data to the DataTable
    private List<TipoAntena> datasource;
     ...
     * @param crudService
     */
    public LazyTipoAntenaDataModel(DataAccessService crudService) {
     ...
     */
    @Override
    public List<TipoAntena> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
        CriteriaBuilder criteriaBuilder = crudService.getCriteriaBuilder();
        CriteriaQuery<TipoAntena> criteriaQuery = criteriaBuilder.createQuery(TipoAntena.class);
        Metamodel metamodel = crudService.getMetamodel();
        EntityType<TipoAntena> entityType = metamodel.entity(TipoAntena.class);
        Root<TipoAntena> from = criteriaQuery.from(entityType);
        List<Order> orders = new ArrayList<Order>();

        // count query
        CriteriaQuery<Long> cqCount = criteriaBuilder.createQuery(Long.class);
        Root<TipoAntena> fromCount = cqCount.from(entityType);
     ...
     */
    @Override
    public Object getRowKey(TipoAntena tipoAntena) {
        return tipoAntena.getId().toString();
    }
     ...
     */
    @Override
    public TipoAntena getRowData(String rowKey) {
        if (datasource == null)
            return null;
        for (TipoAntena tipoAntena : datasource) {
            if (tipoAntena.getId().toString().equals(rowKey))
                return tipoAntena;
        }
        return null;
    }
    ...
    public void setWrappedData(Object list) {
        this.datasource = (List<TipoAntena>) list;
    }
}
