...
public class LazyTipoFeederDataModel extends LazyDataModel<TipoFeeder> implements Serializable {

    // Data Source for binding data to the DataTable
    private List<TipoFeeder> datasource;
     ...
     * @param crudService
     */
    public LazyTipoFeederDataModel(DataAccessService crudService) {
     ...
     */
    @Override
    public List<TipoFeeder> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
        CriteriaBuilder criteriaBuilder = crudService.getCriteriaBuilder();
        CriteriaQuery<TipoFeeder> criteriaQuery = criteriaBuilder.createQuery(TipoFeeder.class);
        Metamodel metamodel = crudService.getMetamodel();
        EntityType<TipoFeeder> entityType = metamodel.entity(TipoFeeder.class);
        Root<TipoFeeder> from = criteriaQuery.from(entityType);
        List<Order> orders = new ArrayList<Order>();

        // count query
        CriteriaQuery<Long> cqCount = criteriaBuilder.createQuery(Long.class);
        Root<TipoFeeder> fromCount = cqCount.from(entityType);
     ...
     */
    @Override
    public Object getRowKey(TipoFeeder tipoFeeder) {
        return tipoFeeder.getId().toString();
    }
     ...
     */
    @Override
    public TipoFeeder getRowData(String rowKey) {
        if (datasource == null)
            return null;
        for (TipoFeeder tipoFeeder : datasource) {
            if (tipoFeeder.getId().toString().equals(rowKey))
                return tipoFeeder;
        }
        return null;
    }
    ...
    public void setWrappedData(Object list) {
        this.datasource = (List<TipoFeeder>) list;
    }
}
