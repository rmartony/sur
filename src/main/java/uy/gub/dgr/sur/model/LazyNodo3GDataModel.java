...
public class LazyNodo3GDataModel extends LazyDataModel<Nodo3G> implements Serializable {

    @Inject
    private Logger log;
    // Data Source for binding data to the DataTable
    private List<Nodo3G> datasource;
     ...
     * @param crudService
     */
    public LazyNodo3GDataModel(DataAccessService crudService) {
     ...
     */
    @Override
    public List<Nodo3G> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
        CriteriaBuilder criteriaBuilder = crudService.getCriteriaBuilder();
        CriteriaQuery<Nodo3G> criteriaQuery = criteriaBuilder.createQuery(Nodo3G.class);
        Metamodel metamodel = crudService.getMetamodel();
        EntityType<Nodo3G> entityType = metamodel.entity(Nodo3G.class);
        Root<Nodo3G> from = criteriaQuery.from(entityType);
        List<Order> orderList = new ArrayList<Order>();

        // count query
        CriteriaQuery<Long> cqCount = criteriaBuilder.createQuery(Long.class);
        Root<Nodo3G> fromCount = cqCount.from(entityType);
...
/*
     ...
     */
    @Override
    public Object getRowKey(Nodo3G sitio) {
     ...
     */
    @Override
    public Nodo3G getRowData(String rowKey) {
        if (datasource == null)
            return null;
        for (Nodo3G nodo3G : datasource) {
            if (nodo3G.getId().toString().equals(rowKey))
                return nodo3G;
        }
        return null;
    }
    ...
    public void setWrappedData(Object list) {
        this.datasource = (List<Nodo3G>) list;
    }
}
