...
public class LazyEstructuraDataModel extends LazyDataModel<Estructura> implements Serializable {

    // Data Source for binding data to the DataTable
    private List<Estructura> datasource;
     ...
     * @param crudService
     */
    public LazyEstructuraDataModel(DataAccessService crudService) {
     ...
     */
    @Override
    public List<Estructura> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
        CriteriaBuilder criteriaBuilder = crudService.getCriteriaBuilder();
        CriteriaQuery<Estructura> criteriaQuery = criteriaBuilder.createQuery(Estructura.class);
        Metamodel metamodel = crudService.getMetamodel();
        EntityType<Estructura> entityType = metamodel.entity(Estructura.class);
        Root<Estructura> from = criteriaQuery.from(entityType);
        List<Order> orders = new ArrayList<Order>();

        // count query
        CriteriaQuery<Long> cqCount = criteriaBuilder.createQuery(Long.class);
        Root<Estructura> fromCount = cqCount.from(entityType);
     ...
     */
    @Override
    public Object getRowKey(Estructura estructura) {
        return estructura.getId().toString();
    }
     ...
     */
    @Override
    public Estructura getRowData(String rowKey) {
        if (datasource == null)
            return null;
        for (Estructura estructura : datasource) {
            if (estructura.getId().toString().equals(rowKey))
                return estructura;
        }
        return null;
    }
    ...
    public void setWrappedData(Object list) {
        this.datasource = (List<Estructura>) list;
    }
}
